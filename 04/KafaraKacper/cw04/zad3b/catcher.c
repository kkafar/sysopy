...
void handle_sig1_kill(int signo, siginfo_t * info, void * ucontext);
void handle_sig1_sigqueue(int signo, siginfo_t * info, void * ucontext);
void handle_sig1_sigrt(int signo, siginfo_t * info, void * ucontext);
...
int main(int argc, char * argv[]) 
{
    ...
    // masking all signals except SIGNAL1 && SIGNAL2
    sigset_t mask;
    ...
    if (strcmp("KILL", argv[1]) == 0) 
    {
        ...
        action_sig1.sa_sigaction = handle_sig1_kill;
    ...
    else if (strcmp("SIGQUEUE", argv[1]) == 0) 
    {
        ...
        action_sig1.sa_sigaction = handle_sig1_sigqueue;
    ...
    else if (strcmp("SIGRT", argv[1]) == 0) 
    {
        ...
        action_sig1.sa_sigaction = handle_sig1_sigrt;
...
void handle_sig1_sigqueue(int signo, siginfo_t * info, void * ucontext) 
{
    ...
    if (sigqueue(sender_pid, SIGNAL1, value) < 0)
    {
        int errnum = errno;
        perror("sigqueue");
        exit(errnum);
