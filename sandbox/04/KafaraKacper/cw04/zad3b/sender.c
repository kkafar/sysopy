...
int no_signals;
volatile sig_atomic_t waiting_for_confirmation = 0;
...
void handle_sig1(int signo, siginfo_t * info, void * ucontext)
{
    // int sigvalue = info->si_int;
    if (waiting_for_confirmation == 1)              /* CONFIRMATION */
    {
        waiting_for_confirmation = 0;
        return;   
    }
    else                                        /* NORMAL SIGNAL FROM CATCHER */
        ++no_received_signals;
}
...
void handle_sig2(int signo)
{
    printf("sender received SIGUSR2/MYSIG2(%d). %d/%d SIGUSR1/MYSIG1 signals were fetched back (%.2lf)%%\n", signo, no_received_signals, no_signals, (double)(no_received_signals) / no_signals * 100);
    exit(EXIT_SUCCESS);
}
...
void killmode(pid_t catcher_pid, sigset_t * mask)
{
    ...
    sigset_t fullmask;
    action_sig1.sa_flags = SA_SIGINFO; /* | SA_NODEFER ?? */
    action_sig1.sa_sigaction = handle_sig1;
    action_sig2.sa_handler = handle_sig2;
    
    if (sigfillset(&action_sig1.sa_mask) < 0 || sigfillset(&action_sig2.sa_mask) < 0 || sigfillset(&fullmask) < 0)
    {
        int errnum = errno;
        perror("sigfillset");
        exit(errnum);
    ...
    {
        // printf("sending singal no %d\n", i);
        if (sigprocmask(SIG_SETMASK, &fullmask, NULL) < 0)
        {
            perror("sigprocmask");
            exit(errno);
        }
        ...
        // printf("waiting for confirmation for signal %d\n", i);
        while (waiting_for_confirmation == 1)
            sigsuspend(mask);
        // printf("confirmation received for signal %d\n", i);
...
void sigqueuemode(pid_t catcher_pid, sigset_t * mask) 
{
    ...
    struct sigaction action_sig1, action_sig2;
    sigset_t fullmask;
    action_sig1.sa_flags = SA_SIGINFO;
    action_sig1.sa_sigaction = handle_sig1;
